...
// MULG
#define MULG_GAME_WIDTH (240)
#define MULG_GAME_HEIGHT (240) /* NES_VISIBLE_HEIGHT */
...
void ili9341_write_frame_scumm(uint8_t* buffer, uint16_t* palette)
{
//return;
...
void ili9341_write_frame_mulg(uint8_t* buffer, uint16_t* myPalette, uint8_t scale)
{
    short x, y;
    ...
    else
    {
        ...
        if (scale)
        {
            ...
            const uint16_t top = (240 - MULG_GAME_HEIGHT) / 2;

            send_reset_drawing((320 / 2) - (displayWidth / 2), top, displayWidth, MULG_GAME_HEIGHT);

            for (y = 0; y < MULG_GAME_HEIGHT; y += LINE_COUNT)
            {
              int linesWritten = 0;
              uint16_t* line_buffer = line_buffer_get();
              ...
              for (short i = 0; i < LINE_COUNT; ++i)
              {
                  if((y + i) >= MULG_GAME_HEIGHT)
                    break;

                  int index = (i) * displayWidth;

                  int bufferIndex = ((y + i) * MULG_GAME_WIDTH) + 4;

                  uint16_t samples[4];
                  for (x = 4; x < MULG_GAME_WIDTH - 4; x += 4)
                  {
                    for (short j = 0; j < 4; ++j)
                    {
        ...
        {
            send_reset_drawing((320 / 2) - (MULG_GAME_WIDTH / 2), (240 / 2) - (MULG_GAME_HEIGHT / 2), MULG_GAME_WIDTH, MULG_GAME_HEIGHT);
            ...
            for (y = 0; y < MULG_GAME_HEIGHT; y += LINE_COUNT)
            {
              ...
              for (short i = 0; i < LINE_COUNT; ++i)
              {
                  ...
                  int index = (i) * MULG_GAME_WIDTH;
                  int bufferIndex = ((y + i) * MULG_GAME_WIDTH);

                  for (x = 0; x < MULG_GAME_WIDTH; ++x)
                  {
              ...
              // display
              send_continue_line(line_buffer, MULG_GAME_WIDTH, linesWritten);
            }
        }
    }
